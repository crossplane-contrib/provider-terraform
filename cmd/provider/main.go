...
func main() {
	var (
		...
		gcInterval               = app.Flag("gc-interval", "Controls how often the workspace directory garbage collector runs. Lower values reclaim disk sooner; higher values reduce filesystem scans. Has no effect on whether a workspace directory can be prematurely removed.").Default("1h").Duration()
	...
	kingpin.FatalIfError(gc.Setup(mgr, workspace.GetTerraformDir(), log, *gcInterval), "cannot setup Workspace garbage collector controller")
