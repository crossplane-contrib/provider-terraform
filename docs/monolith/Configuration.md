...
- **Relabelling mid-apply is supported, including during a Workspace's first apply.** A filtered cache drops a Workspace the instant its label stops matching — the API server delivers "was matching, now isn't" as a `DELETED` watch event — so the outgoing instance finishes its `terraform` run against a Workspace it can no longer read. The reconcilers account for this: they persist Crossplane's create-result bookkeeping (`crossplane.io/external-create-succeeded`) through a client that reads from the API server rather than the cache, and declare the Workspace's external name deterministic so an undeterminable create result is retried rather than refused. Without both, a Workspace relabelled during its first apply is stranded with a `crossplane.io/external-create-pending` annotation that no instance will clear, and the incoming instance silently ignores it — including later spec changes — until someone removes the annotation by hand.
